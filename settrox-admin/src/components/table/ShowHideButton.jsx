...
const ShowHideButton = ({ id, status, category, currencyStatusName }) => {
  ...
  const handleChangeStatus = async (id) => {
    ...
    try {
      ...
      if (location.pathname === "/specification" || specification) {
        const res = await SpecificationServices.updateStatus(id, {
          status: newStatus,
        });
        setIsUpdate(newStatus);
        notifySuccess(res.message);
      }

      if (location.pathname === "/products" || product) {
