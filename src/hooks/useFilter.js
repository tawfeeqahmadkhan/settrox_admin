    ...
    if (brandType) {
      services = services.filter(
        (search) =>
          search?.name[lang]
            ?.toLowerCase()
            ?.includes(brandType?.toLowerCase()) ||
          search?.brand?.toLowerCase().includes(brandType?.toLowerCase())
      );
    }
    //admin Filtering
    if (role) {
  ...
  const handleSelectFile = (e) => {
    ...
    if (file && file.type === "application/json") {
      ...
      fileReader.onload = (e) => {
        ...
        if (location.pathname === "/sub-categories") {
          data = text.map((value) => {
            return {
              _id: value._id,
              id: value.id,
  ...
  const handleUploadMultiple = (e) => {
    ...
    if (selectedFile.length > 1) {
      ...
      if (location.pathname === "/brand") {
        ...
        let brandDataValidation = selectedFile.map((value) =>
          ajv.validate(brandSchema, value)
        );
      
        const isValid = brandDataValidation.every((currentValue) => currentValue === true);
