...
const useProductSubmit = (id) => {
  ...
  const [imageUrlFeature, setImageUrlFeature] = useState([]);
  const [videoUrlFeature, setVideoUrlFeature] = useState("");
  const [tag, setTag] = useState([]); 
  const [description, setDescription] = useState("");
  const [product_feature, setProductFeature] = useState("");
  ...
  const [features, setFeatures] = useState([
    { feature_title: "", feature_subtitle: "", feature_description: "",image: "" },
  ]);
  ...
  const onSubmit = async (data) => {
     ...
     console.log('data features features',features)
     console.log('data videoUrlFeature',videoUrlFeature)
     console.log('data imageUrlFeature',imageUrlFeature)
    try {
      ...
      const productData = {
        ...
        product_feature: handleRemoveEmptyKey({
          [language]: product_feature || "",
        ...
        features_video: videoUrlFeature,
        features: features,
        brand: selectedBrand.map((item) => item._id),
        specification: selectedSpecifications.map((item) => item._id),
        ...
        variants: isCombination ? updatedVariants : [],
		    meta_title: handleRemoveEmptyKey({
          [language]: data.meta_title
        }),
        meta_description: handleRemoveEmptyKey({
          [language]: data.meta_description || ""
        }),
      };
      ...
      if (updatedId) {
        ...
        if (isCombination) {
          ...
          setValue("product_feature", res.product_feature[language ? language : "en"]); 
          setValue("description", res.description[language ? language : "en"]); 
		      setValue("meta_title", res.title[language ? language : "en"]);
          setValue("meta_description", res.description[language ? language : "en"]);
      ...
      (async () => {
        try {
          ...
          if (res) {
			...
			setValue("meta_title", res.meta_title[language ? language : "en"]);
            setValue(
              "meta_description",
              res.meta_description[language ? language : "en"]
            );
            setValue("slug", res.slug);
            setValue("show", res.show);
            ...
            if(res.brand.length > 0){
                res.brand.map((brandRes) => { 
                  brandRes.name = showingTranslateValue(brandRes?.name, lang); 
                  return brandRes;
                });
            }
            if(res.specification.length > 0){
              res.specification.map((specificationRes) => { 
                specificationRes.name = showingTranslateValue(specificationRes?.title, lang); 
            ...
            setSelectedSpecifications(res.specification);
            ...
            setProductFeature(res?.product_feature?.en); 
          }
        } catch (err) {
          notifyError(err?.response?.data?.message || err?.message);
        }
  ...
  useEffect(() => {
    console.log('attribueattribue', attribue);
  
    if (attribue.length > 0) {
      ...
      // Update attTitle only if there's a change
      setAttTitle((prev) => {
        const isSame = JSON.stringify(prev) === JSON.stringify(result);
        return isSame ? prev : result;
      });
  
      console.log('variantsvariants', variants);
  
      const updatedVariants = variants.map(({ attributes, ...rest }) => ({
        ...rest,
        ...attributes,
      }));
      ...
      // Update variants only if there's a change
      setVariants((prev) => {
        const isSame = JSON.stringify(prev) === JSON.stringify(updatedVariants);
        return isSame ? prev : updatedVariants;
      });
  
      const res = Object.keys(Object.assign({}, ...updatedVariants));
      ...
      if (variants.length > 0) {
        ...
        // Update totalStock only if there's a change
        setTotalStock((prev) => (prev === totalStock ? prev : Number(totalStock)));
      }
  
      // Update variantTitle only if there's a change
      setVariantTitle((prev) => {
        const isSame = JSON.stringify(prev) === JSON.stringify(varTitle);
        return isSame ? prev : varTitle;
      });
    }
  ...
  const handleAddAtt = (v, el) => {
    ...
    const attributeArray = result.map((value, index) => {
    ...
    // Update the state with the modified attribute array
  ...
  const handleSelectLanguage = (lang) => {
    ...
    if (Object.keys(resData).length > 0) {
      ...
      setValue("product_feature", resData.product_feature[lang ? lang : "en"]);
    }
  };
  ...
  const handleProductFeatureChange = (content) => {
    setProductFeature(content);
  }; 
 

   // Handle Selected Items
   const handleSelectSpecification = (selectedList) => {
    console.log('selectedList handleSelectSpecification',selectedList)
  ...
  const handleRemoveSpecification = (selectedList) => {
    if (selectedList.length > 0) {
      const updatedList = selectedList.map(item => ({
        _id: item.id,
        name: item.name,
